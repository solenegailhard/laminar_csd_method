'''
Systematic simulations of 2 sources with varying snr
...
- coergister to multilayer surface then 
    - invert using EBB, extract source time series in each layer
...
Here we aim to assess how specific EBB layer can reconstruct 2 simultaneous sources compared to normal EBB
...
from lameg.invert import coregister, invert_ebb, invert_ebb_layer, load_source_time_series
...
def run(
    ...
    try:
        ...
        sim_vx_res = {
            ...
            "thickness": thickness,
            "time": time,
            "ts_ebb": np.zeros((len(sim_vertices), n_layers, len(time))),
        ...
        with spm_context() as spm:
            ...
            for sim_idx, layers in enumerate(sim_layers):
                ...
                # retreive standard ebb time series, only for specified vertex
                ts_ebb, _, _ = load_source_time_series(
                    sim_l_fname, 
                    mu_matrix=MU, 
                ...
                sim_vx_res["ts_ebb"][sim_idx, :, :] = ts_ebb
...
if __name__ == '__main__':
    ...
    out_folder = 'sim_2_simult_sources_snr'
    ...
    output_sim_fname = f"vx{vertex_sim_idx}_2_simult_sources_snr{snr_sim_idx}"
